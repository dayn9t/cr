...
/// 创建CrError的正确状态
#define cr_ok() cr_error(CXR_OK)

/// 创建CrError - 未知错误
#define cr_unknown_error() cr_error(CXR_UNKNOWN_ERR)

/// 创建CrError - 未知异常
#define cr_unknown_exception() cr_error(CXR_UNKNOWN_EXCEPTION)

/// 创建出错信息 - 附带描述信息
#define cr_error_msg(code, msg) cr::error((code), __LINE__, __FILE__, 0, 0, (msg))

/// 创建出错信息 - 从std::exception, TODO: e.what() 可能不是静态生存周期
#define cr_std_exception(e) cr::error(CXR_STD_EXCEPTION, __LINE__, __FILE__, 0, 0, (e).what())
...
#define cr_ensure(cond) while(!(cond)) { throw cr_error_msg(CXR_DBC_ERR, #cond); }
...
#ifndef CXR_FILEPATH_CUT_N
/// 文件路径截调前N段, 避免路径过长, 减少信息泄漏
#define CXR_FILEPATH_CUT_N 3
#endif
